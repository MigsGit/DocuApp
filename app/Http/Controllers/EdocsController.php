...
class EdocsController extends Controller
{
    ...
    public function get_module(Request $request){
        ...
        try {
            ...
            ->addColumn('get_action',function($row){
                // return $row->id;
                return $btn = '<button data-id = "'.$row->id.'"  class="btn btn-outline-info btn-sm" data-toggle="modal" data-target="#modalSave" id="btnEdocs" type="button" title="Edit"><i class="fas fa-edit"></i>Edit</button>';
    ...
    public function saveDocument(Request $request){
        return $this->resource_interface->create( Document::class,$request);
        date_default_timezone_set('Asia/Manila');
        DB::beginTransaction();
        try {
            DB::commit();
